...
| `cd build && ctest --output-on-failure && cmake --build build && find src -name '*.cpp' -o -name '*.h' \| xargs wc -l \| tail -1` | Three-check ritual — run before marking a task done |
| `find src -name '*.cpp' -o -name '*.h' \| xargs wc -l \| tail -1` | LOC baseline check |
