...
- **Multi-condition if-paren trap** — When replacing the last condition in a `if (cond1 || ... || condN)` chain, count closing parens carefully: the final condition needs an extra `)` to close the `if (` itself (pattern: `"...")))`). Missed this in a multi_replace_string_in_file call; caught by the compiler.
