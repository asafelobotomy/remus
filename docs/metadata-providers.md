...
- Notes: RA uses RAHasher MD5 (stored separately as `ra_md5` on file records). The orchestrator passes `raMd5` to RA, not No-Intro `md5`. When Hasheous/compendium already resolved an RA game ID, the provider uses `getById` instead of hash lookup. For disc systems, install the official CLI with `bash scripts/install_rahasher.sh` (→ `data/tools/rahasher/RAHasher`, auto-detected) or set `REMUS_RAHASHER_PATH`. There is no linkable RAHasher library — Remus uses native hash rules for cartridge systems and shells out to the RAHasher binary for PS1/PS2/NDS/GC/Wii/etc.
