...
1. Create `.mcp.json` at the plugin root (uses `mcpServers` key, not `servers`). For Claude-format plugins, use `${CLAUDE_PLUGIN_ROOT}`. For OpenPlugin plugins, replace it with `${PLUGIN_ROOT}`:
...
2. Use the plugin-root token that matches the plugin format for all path references within the plugin
3. Copilot-format plugins do not currently document a plugin-root token in VS Code. Prefer Claude or OpenPlugin format when the server path must resolve inside the plugin directory.
4. Plugin MCP servers start automatically on enable and are implicitly trusted
