...
You are the Doctor agent for Remus.
...
Count total lines of `.github/copilot-instructions.md`. Then for each section, count its lines.
...
Flag: `[CRITICAL]` if any `{{` tokens are found — the file must be fully resolved.
...
2. **Naming conflicts** — if plugins are configured, do any `.github/agents/*.agent.md` files share a
   `name:` with a plugin-contributed agent?
