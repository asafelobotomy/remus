...
void ArtworkController::clearArtworkCache() {
    ...
    const QStringList files = QDir(dir).entryList(QStringList() << QStringLiteral("*.png") << QStringLiteral("*.jpg")
...
bool ArtworkController::refreshArtworkForFile(int fileId, bool requireDownloadableUrl, bool requireConfirmed) {
    ...
    if (existing.releaseDate.isEmpty())
        missing.insert(RELEASE_DATE);
    if (existing.description.isEmpty())
        missing.insert(DESCRIPTION);
    if (existing.genres.isEmpty())
        missing.insert(GENRES);
    if (existing.players == 0)
        missing.insert(PLAYERS);
    if (existing.rating == 0.0f)
    ...
    const GameMetadata enriched = orchestrator->enrichMissingFields(missing, existing, selectBestMatchHash(file),
        deriveMatchingDisplayName(file), systemName, file.crc32, file.md5, file.sha1);

    // Persist newly enriched fields to the stored game record.
    if (matchResult.gameId > 0 && !enriched.title.isEmpty()) {
        m_appController->database()->updateGame(matchResult.gameId, QString(), QString(),
