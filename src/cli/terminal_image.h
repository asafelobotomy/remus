...
private:
    ...
    static bool displayWithChafa(const QString &imagePath, int maxCols) {
        ...
        proc.start("chafa", { "--size", QString("%1x%2").arg(maxCols).arg(maxRows), "--animate", "off", imagePath });
    ...
    static bool displayWithHalfBlocks(const QString &imagePath, int maxCols) {
        ...
        int targetHeight = static_cast<int>(static_cast<double>(img.height()) / img.width() * targetWidth);
        ...
        if (img.height() % 2 != 0) {
            ...
            for (int x = 0; x < img.width(); x++) {
                ...
                out << QStringLiteral("\033[38;2;%1;%2;%3m▀").arg(qRed(px)).arg(qGreen(px)).arg(qBlue(px));
            }
            out << QStringLiteral("\033[0m\n");
        }
