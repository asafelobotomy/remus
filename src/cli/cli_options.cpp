...
void registerAllOptions(QCommandLineParser &parser, QSet<QString> &actionOptions) {
    ...
    addActionOption(
        { { "s", "scan" }, "Scan a directory or file for ROMs (repeatable: -s dir1 -s dir2 -s file.zip)", "path" });
    ...
    addActionOption(QCommandLineOption(
        "check-tools", "Show availability of optional external tools (chdman, dolphin-tool, maxcso)"));
    ...
    addOption(
        QCommandLineOption("igdb-client-secret", "IGDB client secret (env: REMUS_IGDB_CLIENT_SECRET)", "clientSecret"));
    ...
    addOption(
        QCommandLineOption(Constants::Cli::Options::MIN_CONFIDENCE, "Minimum confidence threshold for matches (0-100)",
            "confidence", QString::number(static_cast<int>(Constants::Confidence::Thresholds::DEFAULT_MINIMUM))));
    ...
    addActionOption(QCommandLineOption(
        "enrich", "Enrich matched games with metadata from providers (fills empty description, genre, players)"));
    ...
    addOption(QCommandLineOption(
        "artwork-dir", "Directory to store artwork (default: ~/.local/share/Remus/artwork/)", "directory"));
    addOption(QCommandLineOption(
        "artwork-types", "Types of artwork to download (box|screen|manual|all - default: box)", "types", "box"));

    addActionOption(QCommandLineOption("checksum-verify",
        "Verify specific file checksum (hashes the file as-is; for archives, hashes the container)", "file"));
    addOption(QCommandLineOption("expected-hash", "Expected hash for verification (crc32|md5|sha1)", "hash"));
    addOption(
        QCommandLineOption("hash-type", "Hash type to verify (crc32, md5, sha1 - default: crc32)", "type", "crc32"));

    addActionOption(QCommandLineOption("organize", "Organize and rename files using template", "destination"));
    addOption(QCommandLineOption("template", "Naming template (default: No-Intro standard)", "template",
        Constants::Templates::DEFAULT_NO_INTRO));
    addOption(QCommandLineOption("folder-naming",
        "Sort into system subfolders (none|default|batocera|retropie|emudeck|romm)", "scheme", "none"));
    ...
    addActionOption(QCommandLineOption("bundle",
        "Fetch metadata, download box art, and repack matched ROMs into self-contained archives", "destination"));
    addOption(QCommandLineOption("bundle-format", "Output archive format for bundles (zip|7z, default: zip)", "format",
        Constants::Cli::Defaults::BUNDLE_FORMAT));
    addOption(QCommandLineOption(
        "bundle-art-dir", "Optional pre-downloaded artwork directory (avoids re-downloading box art)", "directory"));
    addOption(QCommandLineOption("bundle-disc-format",
        "Disc media packaging inside bundles (original|chd|rvz, default: chd). When chd is requested, the planner "
        "auto-selects: RVZ for GameCube/Wii, CSO for PSP, CHD for all other disc formats.",
        "format", Constants::Cli::Defaults::BUNDLE_DISC_FORMAT));
    ...
    addActionOption(
        QCommandLineOption("mod-source-url", "List catalog mods whose source URL matches this text", "text"));
    addActionOption(QCommandLineOption(
        "mod-sort", "Sort discovery results by title, author, system, type, format, rating, or downloads", "field"));
    ...
    addOption(
        QCommandLineOption(Constants::Cli::Options::EXPORT_PATH, "Export output path (file or directory)", "path"));
    addOption(
        QCommandLineOption(Constants::Cli::Options::EXPORT_SYSTEMS, "Comma-separated systems to include", "systems"));
    ...
    addActionOption(QCommandLineOption("library",
        ...
        "path"));
    addOption(
        QCommandLineOption("output", "Output directory for --library pipeline (organized library root).", "directory"));

    addActionOption(
        QCommandLineOption("process", "Full pipeline: scan→hash→match→enrich→bundle on a directory", "path"));
    addOption(QCommandLineOption(
        "process-output", "Output directory for --process pipeline bundles/organized files", "directory"));
    ...
    addActionOption(
        QCommandLineOption("extract-archive", "Extract archive (ZIP/7z/RAR/tar/gz/xz via libarchive)", "path"));
    ...
    addOption(QCommandLineOption(
        "rvz-compression", "RVZ compression (zstd, bzip2, lzma, lzma2, none, auto)", "compression", "auto"));
    ...
    addOption(QCommandLineOption(QStringLiteral("file-id"),
        QStringLiteral("Scope hash/match/enrich/bundle/organize/artwork to a specific database file ID (repeatable: "
                       "--file-id 1 --file-id 2)"),
        QStringLiteral("id")));
}
