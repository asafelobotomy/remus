...
int handleMatchCommand(CliContext &ctx) {
    ...
    for (const MatchTask &task : tasks) {
        ...
        GameMetadata metadata
            = orchestrator->searchWithFallback(selectBestHash(file), displayName, systemName, file.crc32, file.md5,
                file.sha1, task.discSerial, file.fileSize, file.raMd5, false, selectContentSha1(file));
