...
static bool isPatchAddressAllowed(const QHostAddress &addr) {
    ...
    if (addr.protocol() == QAbstractSocket::IPv6Protocol) {
        ...
        return !unspec && !((ip.c[0] & 0xFE) == 0xFC) // fc00::/7 unique-local
...
QString ModWorkflowService::resolvePatchPath(const QString &patchUrl, QString &error, ProgressCallback cb) {
    if (patchUrl.isEmpty()) {
...
QString ModWorkflowService::downloadPatch(const QUrl &url, QString &error, ProgressCallback cb) {
    if (cb)
        cb("downloading", 2);
    ...
    for (int redirectCount = 0; redirectCount <= MAX_PATCH_REDIRECTS; ++redirectCount) {
        ...
        if (cb) {
            QObject::connect(reply, &QNetworkReply::downloadProgress, [&cb](qint64 received, qint64 total) {
                if (total > 0) {
