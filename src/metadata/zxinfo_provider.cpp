...
QList<SearchResult> ZXInfoProvider::searchByName(const QString &title,
...
GameMetadata ZXInfoProvider::getById(const QString &id)
{
    ...
    const QUrl url(QStringLiteral("%1/games/%2")
                   .arg(QString::fromLatin1(ZXINFO_BASE_URL), id));
...
QList<GameMetadata> ZXInfoProvider::searchAndFetch(const QString &title, int limit)
{
    ...
    const QJsonArray hits = doc.object()
        .value(QStringLiteral("hits")).toObject()
