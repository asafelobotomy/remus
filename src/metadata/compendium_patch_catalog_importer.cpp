...
namespace CompendiumPatchCatalog {
    ...
    bool importDatFile(QSqlDatabase &database, const QString &datPath, const QString &catalogName, ImportStats &stats,
        QString &error) {
        ...
        deleteEntries.prepare(
            QStringLiteral("DELETE FROM patch_entries WHERE source_id IN "
        ...
        for (const ClrMameProEntry &entry : entries) {
            const PatchedRomInfo parsed = PatchedRomParser::parse(entry.gameName);
            const QString patchName = entry.patchUrl.isEmpty() ? parsed.patchName : entry.patchUrl;
