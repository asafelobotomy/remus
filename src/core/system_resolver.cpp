...
{
    // Map No-Intro / Redump canonical DAT names to system IDs.
    // This is a static map so it's built once and reused.
    ...
    // Substring match — handle DAT names that contain the canonical name
    for (auto it = datNameMap.constBegin(); it != datNameMap.constEnd(); ++it) {
