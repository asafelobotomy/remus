...
ConversionPlanner::Plan ConversionPlanner::plan(const Request &request)
{
    ...
    if (request.systemId <= 0 || extension.isEmpty()) {
        return makePlan(FormatRole::Deferred,
    ...
    if (!isRecognizedForSystem(request.systemId, extension)) {
        return makePlan(FormatRole::Deferred,
                        ...
                        QStringLiteral("%1 is not registered as a known format for %2.")
                            .arg(extension, systemName));
    }

    if (isArchiveOnlySystem(request.systemId)) {
        return makePlan(FormatRole::ArchiveOnly,
                        PlannedAction::ArchiveAsIs,
                        request.systemId,
                        {},
                        {},
                        QStringLiteral("Archive the payload as-is; no conversion backend is required."),
                        QStringLiteral("%1 media does not have a canonical conversion target in Remus, so %2 stays in the archive-only track.")
    ...
    if (isDeferredSystem(request.systemId)) {
        return makePlan(FormatRole::Deferred,
                        ...
                        QStringLiteral("%1 remains in the deferred policy gate, so %2 is tracked but not yet normalized or exported.")
                            .arg(systemName, extension));
    }

    if (isChdCanonicalSystem(request.systemId)) {
        if (extension == CHD || isCanonicalPlaylistExtension(request.systemId, extension)) {
            return makePlan(FormatRole::Canonical,
                            PlannedAction::KeepAsIs,
                            request.systemId,
                            {},
                            {},
                            noFallbackRequired(),
                            QStringLiteral("%1 is already in the canonical CHD workflow for %2.")
                                .arg(extension, systemName));
        }

        if (request.systemId == Systems::ID_PSX && extension == pbpExtension()) {
            const PlannedAction action = request.intent == PlanningIntent::ExplicitExport
                ? PlannedAction::ExportPbp
                : PlannedAction::NoOp;
            return makePlan(FormatRole::ExportOnly,
                            action,
                            request.systemId,
                            {},
                            {QStringLiteral("PSXPackager")},
                            keepOriginalUntilToolAvailable(QStringLiteral("PSXPackager")),
                            QStringLiteral("PBP is an explicit PlayStation export format. Remus keeps %1 as the canonical archive and does not promote %2 into the main library target.")
                                .arg(CHD, extension));
        }

        if (isChdConvertibleExtension(extension)) {
            return makePlan(FormatRole::Canonical,
                            PlannedAction::ConvertToChd,
                            request.systemId,
                            {},
                            {QStringLiteral("chdman")},
        ...
        if (isChdNormalizationExtension(request.systemId, extension)) {
            ...
            if (request.systemId == Systems::ID_PS2 && extension == CSO) {
                ...
                reason = QStringLiteral("%1 is treated as a normalization-first input for %2 before the canonical CHD path.")
                    .arg(extension, systemName);
            }
            ...
            return makePlan(FormatRole::NormalizationOnly,
                            PlannedAction::NormalizeToIso,
                            request.systemId,
                            ISO,
                            tools,
                            chainFallback(tools),
        ...
        if (isPs2ArchiveOnlyExtension(extension)) {
            return makePlan(FormatRole::ArchiveOnly,
                            ...
                            QStringLiteral("%1 executables stay in the archive-only track even though the main disc workflow is CHD.")
                                .arg(systemName));
        }
    }
    ...
    if (isRvzCanonicalSystem(request.systemId)) {
        if (extension == RVZ) {
            return makePlan(FormatRole::Canonical,
                            ...
                            {},
                            noFallbackRequired(),
                            QStringLiteral("%1 is already in the canonical RVZ workflow for %2.")
                                .arg(extension, systemName));
        }

        if (extension == ISO || extension == GCM) {
            return makePlan(FormatRole::Canonical,
                            PlannedAction::ConvertToRvz,
                            request.systemId,
                            {},
                            {QStringLiteral("dolphin-tool")},
        ...
        if (isRvzNormalizationExtension(extension)) {
            return makePlan(FormatRole::NormalizationOnly,
                            ...
                            QStringLiteral("%1 is a normalization-first input for %2. Normalize to ISO before the canonical RVZ path.")
                                .arg(extension, systemName));
        }
    ...
    if (request.systemId == Systems::ID_PSP) {
        if (extension == CSO) {
            return makePlan(FormatRole::Canonical,
                            ...
                            {},
                            noFallbackRequired(),
                            QStringLiteral("%1 is already in the canonical CSO workflow for %2.")
                                .arg(extension, systemName));
        }

        if (extension == ISO) {
            return makePlan(FormatRole::Canonical,
                            PlannedAction::ConvertToCso,
                            request.systemId,
                            {},
                            {QStringLiteral("maxcso")},
        ...
        if (isPspNormalizationExtension(extension)) {
            return makePlan(FormatRole::NormalizationOnly,
                            ...
                            QStringLiteral("%1 is accepted as an ingest format for PSP, but the canonical compact output remains CSO.")
                                .arg(extension));
        }
    }
