...
bool ConversionPlanner::Plan::isValid() const {
    return !ConversionPlanner::toString(role).isEmpty() && !ConversionPlanner::toString(action).isEmpty()
        && !fallbackBehavior.trimmed().isEmpty() && !reason.trimmed().isEmpty();
}
...
ConversionPlanner::Plan ConversionPlanner::plan(const Request &request) {
    ...
    if (isArchiveOnlySystem(request.systemId)) {
        return makePlan(FormatRole::ArchiveOnly, PlannedAction::ArchiveAsIs, request.systemId, { }, { },
            ...
            QStringLiteral(
                "%1 media does not have a canonical conversion target in Remus, so %2 stays in the archive-only track.")
                .arg(systemName, extension));
    }
    ...
    if (isDeferredSystem(request.systemId)) {
        return makePlan(FormatRole::Deferred, PlannedAction::Deferred, request.systemId, { }, { },
            ...
            QStringLiteral(
                "%1 remains in the deferred policy gate, so %2 is tracked but not yet normalized or exported.")
                .arg(systemName, extension));
    }
    ...
    if (isChdCanonicalSystem(request.systemId)) {
        ...
        if (request.systemId == Systems::ID_PSX && extension == PBP) {
            const PlannedAction action
                = request.intent == PlanningIntent::ExplicitExport ? PlannedAction::ExportPbp : PlannedAction::NoOp;
            return makePlan(FormatRole::ExportOnly, action, request.systemId, { }, { QStringLiteral("PSXPackager") },
                ...
                QStringLiteral("PBP is an explicit PlayStation export format. Remus keeps %1 as the canonical archive "
                               "and does not promote %2 into the main library target.")
                    .arg(CHD, extension));
        }

        if (isChdConvertibleExtension(extension)) {
            return makePlan(FormatRole::Canonical, PlannedAction::ConvertToChd, request.systemId, { },
                { QStringLiteral("chdman") }, keepOriginalUntilToolAvailable(QStringLiteral("chdman")),
                QStringLiteral("%1 should convert %2 into CHD as the canonical archive format.")
                    .arg(systemName, extension));
        }
        ...
        if (isChdNormalizationExtension(request.systemId, extension)) {
            ...
            if (request.systemId == Systems::ID_PS2 && extension == CSO) {
                reason = QStringLiteral("PlayStation 2 CSO files are optional export artifacts. Normalize %1 back to "
                                        "ISO before the canonical CHD path.")
                             .arg(extension);
            } else if (request.systemId == Systems::ID_PSX && extension == ECM) {
                reason = QStringLiteral("PlayStation ECM payloads are compatibility inputs. Normalize %1 back to "
                                        "source disc assets before the canonical CHD path.")
                             .arg(extension);
            } else if (request.systemId == Systems::ID_DREAMCAST && extension == CDI) {
                reason = QStringLiteral("Dreamcast CDI images are accepted for ingest, but they should normalize to an "
                                        "ISO-like intermediate before the canonical CHD path.");
            } else {
                reason = QStringLiteral(
                    "%1 is treated as a normalization-first input for %2 before the canonical CHD path.")
                             .arg(extension, systemName);
            }
            ...
            return makePlan(FormatRole::NormalizationOnly, PlannedAction::NormalizeToIso, request.systemId, ISO, tools,
                chainFallback(tools), reason);
        }

        if (isPs2ArchiveOnlyExtension(extension)) {
            return makePlan(FormatRole::ArchiveOnly, PlannedAction::ArchiveAsIs, request.systemId, { }, { },
                QStringLiteral("Archive the payload as-is; %1 is not part of the canonical disc conversion path.")
                    .arg(extension),
                QStringLiteral(
                    "%1 executables stay in the archive-only track even though the main disc workflow is CHD.")
                    .arg(systemName));
        }
    }
    ...
    if (isRvzCanonicalSystem(request.systemId)) {
        ...
        if (extension == ISO || extension == GCM) {
            return makePlan(FormatRole::Canonical, PlannedAction::ConvertToRvz, request.systemId, { },
                { QStringLiteral("dolphin-tool") }, keepOriginalUntilToolAvailable(QStringLiteral("dolphin-tool")),
                QStringLiteral("%1 should convert %2 into RVZ as the canonical archive format.")
                    .arg(systemName, extension));
        }
        if (isRvzNormalizationExtension(extension)) {
            return makePlan(FormatRole::NormalizationOnly, PlannedAction::NormalizeToIso, request.systemId, ISO,
                ...
                QStringLiteral(
                    "%1 is a normalization-first input for %2. Normalize to ISO before the canonical RVZ path.")
                    .arg(extension, systemName));
        }
        if (isRvzArchiveOnlyExtension(extension)) {
            return makePlan(FormatRole::ArchiveOnly, PlannedAction::ArchiveAsIs, request.systemId, { }, { },
                QStringLiteral("Archive the payload as-is; no disc conversion backend is required."),
                QStringLiteral("%1 is not a canonical disc-image input for %2, so it stays in the archive-only track.")
                    .arg(extension, systemName));
        }
    }
    ...
    if (request.systemId == Systems::ID_PSP) {
        ...
        if (extension == ISO) {
            return makePlan(FormatRole::Canonical, PlannedAction::ConvertToCso, request.systemId, { },
                { QStringLiteral("maxcso") }, keepOriginalUntilToolAvailable(QStringLiteral("maxcso")),
                QStringLiteral("%1 should convert %2 into CSO as the canonical compact output.")
                    .arg(systemName, extension));
        }
        if (extension == PBP) {
            return makePlan(FormatRole::ExportOnly, PlannedAction::NoOp, request.systemId, { },
                { QStringLiteral("PSXPackager") }, keepOriginalUntilToolAvailable(QStringLiteral("PSXPackager")),
                QStringLiteral("PBP is treated as a compatibility package around PSP workflows, not as the canonical "
                               "PSP library format."));
        }
        if (isPspNormalizationExtension(extension)) {
            return makePlan(FormatRole::NormalizationOnly, PlannedAction::NormalizeToIso, request.systemId, ISO,
                ...
                QStringLiteral(
                    "%1 is accepted as an ingest format for PSP, but the canonical compact output remains CSO.")
                    .arg(extension));
        }
    }
    ...
    return makePlan(FormatRole::Deferred, PlannedAction::Deferred, request.systemId, { }, { },
        ...
        QStringLiteral("%1 on %2 is recognized but not yet mapped to a stable planner workflow.")
            .arg(extension, systemName));
}
