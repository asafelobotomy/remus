...
private slots:
    ...
    void testComplexExample() {
        ...
        QCOMPARE(FilenameNormalizer::normalize("Sonic The Hedgehog (USA, Europe).md"), QString("Sonic The Hedgehog"));

        // Another complex example
        QCOMPARE(FilenameNormalizer::normalize("Shin Megami Tensei 1 (English Addendum 1.0)[Automap].smc"),
            QString("Shin Megami Tensei 1"));

        // Multiple brackets
        QCOMPARE(FilenameNormalizer::normalize("Doom [!] [Classics].wad"), QString("Doom"));
    }
    ...
    void testDisplayNameWithoutExtensionPreservesVersionDots() {
        QCOMPARE(
            FilenameNormalizer::normalize("Dragon Quest III (English v2.0)[Addendum]"), QString("Dragon Quest III"));
    }
};
