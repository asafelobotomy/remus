...
static SourceRecordEnvelope makeRec(const QString &sha1 = { }, const QString &crc32 = { }, const QString &md5 = { }) {
...
void CompendiumIdentityLinkerTest::loadFromDatabase_populatesMapsFromExistingDb() {
    ...
    const QString connName = QStringLiteral("linker_test_%1").arg(QDateTime::currentMSecsSinceEpoch());
    ...
    QVERIFY(exec(QStringLiteral("CREATE TABLE games ("
                                "game_id TEXT PRIMARY KEY, system_id INTEGER NOT NULL,"
                                " canonical_title TEXT NOT NULL, primary_region_code TEXT)")));
    QVERIFY(exec(QStringLiteral("CREATE TABLE game_signatures ("
                                "signature_id INTEGER PRIMARY KEY AUTOINCREMENT, game_id TEXT NOT NULL,"
                                " hash_type TEXT NOT NULL, hash_value TEXT NOT NULL)")));
    QVERIFY(exec(QStringLiteral("CREATE TABLE game_serials ("
                                "serial_id INTEGER PRIMARY KEY AUTOINCREMENT, game_id TEXT NOT NULL,"
                                " serial_value TEXT NOT NULL)")));
    QVERIFY(exec(QStringLiteral("CREATE TABLE game_names ("
                                "game_name_id INTEGER PRIMARY KEY AUTOINCREMENT, game_id TEXT NOT NULL,"
                                " name_text TEXT NOT NULL)")));

    QVERIFY(exec(QStringLiteral("INSERT INTO games (game_id, system_id, canonical_title, primary_region_code)"
                                " VALUES ('existing-1', 1, 'Test Game', 'USA')")));
    QVERIFY(exec(QStringLiteral("INSERT INTO game_signatures (game_id, hash_type, hash_value)"
                                " VALUES ('existing-1', 'md5', 'testmd5hashvalue')")));
    QVERIFY(exec(QStringLiteral("INSERT INTO game_names (game_id, name_text)"
