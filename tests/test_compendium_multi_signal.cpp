...
void CompendiumMultiSignalTest::orchestratorUsesMultiSignalWhenHashMisses() {
    ...
    QVERIFY(execSql(db,
        QStringLiteral("INSERT OR REPLACE INTO systems (system_id, internal_name, display_name) "
    ...
    const GameMetadata result
        = orchestrator.searchWithFallback(QStringLiteral("BADHASH"), QStringLiteral("Super Mario Bros. (World).nes"),
            QStringLiteral("NES"), QString(), QString(), QString(), QString(), 40976);
...
void CompendiumMultiSignalTest::matchROM_filenameSizeFallback() {
    ...
    const QString payload
        = QStringLiteral(R"({"rom_name":"Sonic The Hedgehog (USA, Europe).md","size":524288,"crc32":"F9394E97"})");
...
void CompendiumMultiSignalTest::matchROM_serialOnly() {
    ...
    QVERIFY(execSql(db,
        QStringLiteral("INSERT INTO games (game_id, system_id, canonical_title, canonical_confidence) "
            ...
            .arg(gcSystemId)));
    QVERIFY(execSql(db,
        QStringLiteral("INSERT INTO game_serials (game_id, serial_value, source_entry_key, confidence) "
