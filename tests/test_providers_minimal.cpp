...
public:
    ...
    bool proxyEnabledForTest() const { return metadataProxyEnabled(); }
};
...
void ProvidersMinimalTest::igdbGetByIdRejectsNonNumericId()
{
    ...
    QTest::ignoreMessage(QtWarningMsg,
        QRegularExpression(QStringLiteral("IGDB::getById.*rejected non-numeric id.*not-a-number")));
...
void ProvidersMinimalTest::igdbGetByIdRejectsZeroOrNegativeId()
{
    ...
    QTest::ignoreMessage(QtWarningMsg,
        QRegularExpression(QStringLiteral("IGDB::getById.*rejected non-numeric id.*\\b0\\b")));
    const GameMetadata md0 = provider.getById(QStringLiteral("0"));
    QVERIFY(md0.title.isEmpty());
    QVERIFY(errorSpy.isEmpty());

    QTest::ignoreMessage(QtWarningMsg,
        QRegularExpression(QStringLiteral("IGDB::getById.*rejected non-numeric id.*-1")));
...
void ProvidersMinimalTest::igdbGetArtworkRejectsNonNumericId()
{
    ...
    QTest::ignoreMessage(QtWarningMsg,
        QRegularExpression(QStringLiteral("IGDB::getArtwork.*rejected non-numeric id.*abc")));
...
void ProvidersMinimalTest::igdbFetchPlatformSlugRejectsInvalidSlug()
{
    ...
    QTest::ignoreMessage(QtWarningMsg,
        QRegularExpression(QStringLiteral("IGDB::fetchGamesByPlatformSlug.*rejected invalid slug.*NES")));
    ...
    QTest::ignoreMessage(QtWarningMsg,
        QRegularExpression(QStringLiteral("IGDB::fetchGamesByPlatformSlug.*rejected invalid slug")));
...
void ProvidersMinimalTest::hasheousConstructorIgnoresQSettings()
{
    ...
    QSettings s(QString::fromLatin1(Constants::SETTINGS_ORGANIZATION),
    ...
    QVERIFY2(!provider.proxyEnabledForTest(),
             "HasheousProvider constructor must not read API key from QSettings");
