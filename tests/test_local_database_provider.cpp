...
void LocalDatabaseProviderTest::searchByNameFindsSerialOnlyEntries() {
    ...
    QVERIFY(writeFile(metadataRoot + QStringLiteral("/developer/Nintendo - GameCube.dat"),
        QByteArrayLiteral("clrmamepro (\n"
                          "  name \"Nintendo - GameCube\"\n"
                          ")\n"
                          "game (\n"
                          "  name \"TTYD Alias\"\n"
    ...
    const QList<SearchResult> results
        = provider.searchByName(QStringLiteral("Paper Mario"), QStringLiteral("GameCube"));
...
void LocalDatabaseProviderTest::getByHashMergesCrcAndSerialEnrichment() {
    ...
    QVERIFY(writeFile(datPath,
        QByteArrayLiteral("clrmamepro (\n"
                          ...
                          "    name \"Paper Mario: The Thousand-Year Door (USA)\"\n"
                          "    serial \"G8ME01\"\n"
                          "    rom ( name \"Paper Mario - The Thousand-Year Door.iso\" size 1459978240 crc AABB1122 "
                          "serial \"G8ME01\" )\n"
                          ")\n")));

    const QString metadataRoot = tmpDir.filePath(QStringLiteral("metadata"));
    QVERIFY(writeFile(metadataRoot + QStringLiteral("/genre/Nintendo - GameCube.dat"),
        QByteArrayLiteral("clrmamepro (\n"
                          "  name \"Nintendo - GameCube\"\n"
                          ")\n"
                          "game (\n"
