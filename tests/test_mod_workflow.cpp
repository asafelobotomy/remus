...
void ModWorkflowTest::findModsForRom_hashMatch() {
    ...
    auto mods = provider.findModsForRom(QString(), QString(), "da39a3ee5e6b4b0d3255bfef95601890afd80709");
...
void ModWorkflowTest::findModsForRom_noMatch() {
    ...
    auto mods = provider.findModsForRom(
        "00000000", "00000000000000000000000000000000", "0000000000000000000000000000000000000000");
...
void ModWorkflowTest::loadFromUrl_privateIpHostRejected() {
    ...
    for (const QString &urlStr : privateUrls) {
        ...
        ModCatalogProvider provider;
        QVERIFY2(!provider.loadFromUrl(url, true), qPrintable(QStringLiteral("Expected rejection for %1").arg(urlStr)));
        QVERIFY2(provider.lastError().contains("disallowed", Qt::CaseInsensitive)
                || provider.lastError().contains("host", Qt::CaseInsensitive),
            qPrintable(QStringLiteral("Unexpected error for %1: %2").arg(urlStr, provider.lastError())));
    }
}
