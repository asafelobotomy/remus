...
{
    return execSql(db, QStringLiteral("CREATE TABLE systems (system_id INTEGER PRIMARY KEY, internal_name TEXT NOT NULL UNIQUE, display_name TEXT NOT NULL, libretro_name TEXT)"))
...
int insertFact(QSqlDatabase &db,
...
bool resolveFact(QSqlDatabase &db,
                 const QString &gameId,
                 const QString &fieldName,
                 int factId)
{
    QSqlQuery insert(db);
    insert.prepare(QStringLiteral(
        "INSERT INTO canonical_resolution (game_id, field_name, selected_fact_id, resolved_by_rule) "
        "VALUES (?, ?, ?, 'test_rule')"));
    insert.addBindValue(gameId);
    insert.addBindValue(fieldName);
...
{
    if (!execSql(db, QStringLiteral("INSERT INTO systems (system_id, internal_name, display_name, libretro_name) VALUES (4, 'GameCube', 'Nintendo GameCube', 'Nintendo - Nintendo GameCube')"))) {
        return false;
    }
    ...
    const int developerFact = insertFact(db, QStringLiteral("game-1"), QStringLiteral("developer"), QStringLiteral("Intelligent Systems"));
    const int releaseYearFact = insertFact(db, QStringLiteral("game-1"), QStringLiteral("release_year"), QStringLiteral("2004"));
    const int playersFact = insertFact(db, QStringLiteral("game-1"), QStringLiteral("players_max"), QStringLiteral("1"));
    const int descriptionFact = insertFact(db, QStringLiteral("game-1"), QStringLiteral("description"), QStringLiteral("A turn-based adventure across the Mushroom Kingdom."));
    const int genreFact = insertFact(db, QStringLiteral("game-1"), QStringLiteral("genre"), QStringLiteral("Role-Playing"));
...
QString createFixtureDatabase()
{
    ...
    const QString connectionName = QStringLiteral("compendium_fixture_%1")
...
void CompendiumProviderTest::searchByNameUsesAliasAndFilters()
{
    ...
    const QList<SearchResult> wrongRegion = provider.searchByName(
        QStringLiteral("TTYD"),
        QStringLiteral("GameCube"),
        QStringLiteral("JPN"));
...
void CompendiumProviderTest::getBySerialReturnsCanonicalMetadata()
{
    ...
    const GameMetadata metadata = provider.getBySerial(QStringLiteral(" G8ME01 "),
                                                       QStringLiteral("GameCube"));
    QCOMPARE(metadata.id, QStringLiteral("game-1"));
    QCOMPARE(metadata.title, QStringLiteral("Paper Mario: The Thousand-Year Door"));
    QCOMPARE(metadata.system, QStringLiteral("GameCube"));
...
void CompendiumProviderTest::getArtworkBuildsLibretroThumbnailUrls()
{
    ...
    QCOMPARE(artwork.boxFront.toString(),
             QStringLiteral("https://thumbnails.libretro.com/Nintendo - Nintendo GameCube/Named_Boxarts/Paper Mario_ The Thousand-Year Door.png"));
